...
describe('validation.ts', () => {
  describe('validateAndNormalizeTrackId', () => {
    ...
    it('正常系: 数値の track_id を受け取ると成功', () => {
      ...
      expect(result).toEqual({ success: true, trackId: 12345 });
    });

    it('正常系: 数値文字列の track_id を受け取ると成功', () => {
      const result = validateAndNormalizeTrackId({ track_id: '98765' });
      expect(result).toEqual({ success: true, trackId: 98765 });
    });

    it('正常系: 1 は有効な正の整数', () => {
      const result = validateAndNormalizeTrackId({ track_id: 1 });
      expect(result).toEqual({ success: true, trackId: 1 });
    });

    it('正常系: 大きな安全な整数値も受け入れる', () => {
      const result = validateAndNormalizeTrackId({ track_id: Number.MAX_SAFE_INTEGER });
      expect(result).toEqual({ success: true, trackId: Number.MAX_SAFE_INTEGER });
    });
    ...
    it('異常系: body が undefined の場合', () => {
      const result = validateAndNormalizeTrackId(undefined);
      expect(result.success).toBe(false);
      expect(result).toEqual({ success: false, error: 'track_id is required', status: 400 });
    });

    it('異常系: body が数値の場合', () => {
      const result = validateAndNormalizeTrackId(123);
      expect(result.success).toBe(false);
      expect(result).toEqual({ success: false, error: 'track_id is required', status: 400 });
    });

    it('異常系: body が文字列の場合', () => {
      const result = validateAndNormalizeTrackId('not-object');
      expect(result.success).toBe(false);
      expect(result).toEqual({ success: false, error: 'track_id is required', status: 400 });
    });
    ...
    it('異常系: track_id が 0 の場合', () => {
      ...
      expect(result).toEqual({
        success: false,
        error: 'track_id must be a valid positive integer',
        status: 400,
      });
    });
    ...
    it('異常系: track_id が MAX_SAFE_INTEGER より大きい場合', () => {
      const result = validateAndNormalizeTrackId({ track_id: Number.MAX_SAFE_INTEGER + 1 });
      expect(result.success).toBe(false);
      expect(result).toEqual({
        success: false,
        error: 'track_id must be a valid positive integer',
